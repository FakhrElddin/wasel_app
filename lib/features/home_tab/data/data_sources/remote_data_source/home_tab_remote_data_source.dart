...
import 'package:wasel_app/features/home_tab/data/models/books_response_dm.dart';
...
class HomeTabRemoteDataSourceImpl implements HomeTabRemoteDataSource {
  ...
  @override
  Future<Either<Failures, BooksResponseDm>> getBooks() async {
    try {
      final List<ConnectivityResult> connectivityResult = await Connectivity()
          .checkConnectivity();
      if (connectivityResult.contains(ConnectivityResult.mobile) ||
          connectivityResult.contains(ConnectivityResult.wifi)) {
        var response = await apiManager.getData(
          endPoint: EndPoints.booksEndPoint,
        );
        var booksResponse = BooksResponseDm.fromJson(response.data);
        if (response.statusCode! >= StatusCode.statusCode200 &&
            response.statusCode! < StatusCode.statusCode300) {
          return Right(booksResponse);
        } else {
          return Left(ServerError(errorMessage: booksResponse.message!));
        }
      } else {
        return Left(NetworkError());
      }
